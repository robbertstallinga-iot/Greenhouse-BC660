#ifndef __Quectel_BC660_custom_h__
#define __Quectel_BC660_custom_h__
...
class QuectelBC660 {
    public:
        ...
        // MQTT
        bool configureMQTTSSL(int tcpConnectID, bool sslEnable, int sslContextID, int sslConnectID);
        bool openMQTT(uint8_t TCPconnectID, const char* host, uint16_t port);
        bool closeMQTT();
        bool connectMQTT(const char* clientID, const char* username, const char* password);
        bool subscribeToTopic(int tcpConnectID, int msgID, const char* topic, int qos);
